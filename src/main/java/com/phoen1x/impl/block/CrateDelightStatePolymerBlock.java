...
public record CrateDelightStatePolymerBlock(Map<BlockState, BlockState> map, FactoryBlock fallback) implements FactoryBlock, PolymerTexturedBlock, BSMMParticleBlock {

    public static CrateDelightStatePolymerBlock of(Block block, BlockModelType type) {
        return of(block, type, CrateDelightBaseFactoryBlock.BARRIER, x -> true);
    }
    public static CrateDelightStatePolymerBlock of(Block block, BlockModelType type, FactoryBlock fallback, Predicate<BlockState> canUseBlock) {
        var id = Registries.BLOCK.getId(block);

        var path = FabricLoader.getInstance().getModContainer("cratedelight").get()
        ...
        try {
            ...
            return new CrateDelightStatePolymerBlock(map, fallback);
        } catch (Throwable e) {
            return null;
        }
    }
